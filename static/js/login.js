...
function validateSchoolEmail(email) {
  ...
  const isSchoolDomain = SCHOOL_EMAIL_DOMAINS.some(d =>
    domain === d.toLowerCase() || domain.endsWith('.' + d.toLowerCase())
  );
  return STRICT_EMAIL_DOMAIN ? isSchoolDomain : (isSchoolDomain || ALLOWED_PUBLIC_DOMAINS.includes(domain));
}
...
function redirectByRole(role, status) {
  ...
  if (s !== 'active') { window.location.replace('pending-approval.html'); return; }
  ...
  else                         window.location.replace('pending-approval.html');
}
...
setInterval(() => { currentSlide = (currentSlide + 1) % 6; updateDots(currentSlide); }, 5000);
dots.forEach(dot => dot.addEventListener('click', () => {
  currentSlide = +dot.dataset.index; updateDots(currentSlide);
}));
...
document.getElementById('showSignup')?.addEventListener('click', e => {
  ...
  if (emailInput) emailInput.placeholder = `your.name@${SCHOOL_EMAIL_DOMAINS[0]}`;
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginEmail)  loginEmail.placeholder  = `your.name@${SCHOOL_EMAIL_DOMAINS[0]}`;
  if (signupEmail) signupEmail.placeholder = `your.name@${SCHOOL_EMAIL_DOMAINS[0]}`;
});
...
document.getElementById('signupForm')?.addEventListener('submit', async e => {
  ...
  if (!validateSchoolEmail(email)) return showMsg('signupMessage', `Invalid email domain. ${getEmailDomainHelp()}`, 'error');
  if (password.length < 10)        return showMsg('signupMessage', 'Password must be at least 10 characters.', 'error');
  if (password !== confirm)        return showMsg('signupMessage', 'Passwords do not match.', 'error');
  ...
  if (USE_BACKEND) {
    try {
      ...
      setTimeout(() => document.getElementById('showLogin')?.click(), 2500);
    } catch (err) {
      showMsg('signupMessage', err.message || 'Signup failed. Try again later.', 'error');
    }
  } else {
    if (Users.exists(email)) return showMsg('signupMessage', 'Email already registered.', 'error');
...
document.getElementById('loginForm')?.addEventListener('submit', async e => {
  ...
  if (USE_BACKEND) {
    try {
      ...
      // ── FIX: save the auth token immediately after login ──────────
      ...
      // Load permissions using the token
      let permissions = {};
      try {
        ...
        if (roleRes.ok) {
          ...
          if (matched?.permissions) {
            permissions = typeof matched.permissions === 'string'
              ? JSON.parse(matched.permissions)
              : matched.permissions;
          }
        }
      } catch (err) {
      ...
      // Load fresh profile using the token
      try {
        const profileRes = await fetch(`${API_BASE_URL}/api/profile`, {
          headers: { 'Authorization': `Bearer ${token}` }  // FIX: token-based, not email param
        });
        if (profileRes.ok) {
          const profileData = await profileRes.json();
          if (profileData.role)   freshRole   = profileData.role.toLowerCase().trim();
          if (profileData.status) freshStatus = profileData.status.toLowerCase().trim();
        }
      } catch (err) {
...
document.querySelector('.forgot-password')?.addEventListener('click', async e => {
  ...
  if (USE_BACKEND) {
    ...
    const newPass = prompt('Enter your new password (min 10 chars):');
    if (!newPass || newPass.length < 10) return showMsg('loginMessage', 'Password too short.', 'error');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateStrongPassword() {
    ...
    return (word + num1 + num2 + spec + extra1 + extra2 + extra3)
      .split('').sort(() => Math.random() - 0.5).join('');
  }

  function validateGenerated(pwd) {
    return pwd.length >= 10 && /[A-Z]/.test(pwd) && /[a-z]/.test(pwd) &&
  ...
  document.getElementById('generatePasswordBtn')?.addEventListener('click', () => {
    showGeneratedPassword(getValidPassword());
  });
